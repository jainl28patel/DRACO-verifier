...
// Tests for lookup in hash map with a half symbolic struct as key
// And then updating the value which is a struct
SEC("xdp_fw")
int xdp_fw_prog(struct xdp_md *ctx) {
	...
	uint64_t nh_off = 0;
	struct otherStruct *value;
	struct someStruct myStruct;
	klee_make_symbolic(&myStruct, sizeof(struct someStruct), "my_struct");
	myStruct.in_port = 257;
	...
	value = bpf_map_lookup_elem(&blacklist, &myStruct);
	if (!value)
		return XDP_DROP;
