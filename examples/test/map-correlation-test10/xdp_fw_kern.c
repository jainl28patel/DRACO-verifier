...
// Tests for lookup in hash map, and then compare returned value
SEC("xdp_fw")
int xdp_fw_prog(struct xdp_md *ctx) {
	...
	otherValue = bpf_map_lookup_elem(&otherlist, value);

	if (!otherValue)
		return XDP_DROP;
	
	finalValue = bpf_map_lookup_elem(&finalList, &otherValue);
...
int main(int argc, char** argv) {
	...
	BPF_MAP_INIT(&finalList, "finalList_map", "", "");
