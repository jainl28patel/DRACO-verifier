...
// Tests that reassigning value doesn't lead to map correlation
SEC("xdp_fw")
int xdp_fw_prog(struct xdp_md *ctx) {
	...
	__u32 check;
	__u32 *value;
	__u32 keyToUpdate = 19;
	int branching = klee_int("branching");
	...
	if (branching > 5) {
		check += *value;
		if (bpf_map_update_elem(&otherlist, &keyToUpdate, &check, BPF_ANY) < 0) 
			return XDP_DROP;
	} else {
		if (bpf_map_update_elem(&otherlist, &keyToUpdate, &check, BPF_ANY) < 0) 
			return XDP_DROP;
