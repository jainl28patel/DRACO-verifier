...
static __always_inline int handle_syn(struct xdp_md *ctx, struct ethhdr *ethh,
  ...
  BPF_ASSERT_END_EQ(&ctx->data, __u32, ctx->data - sizeof(struct redir_opt));
  BPF_ASSERT_CONSTANT(&ctx->data_end, sizeof(__u32));

  BPF_ASSERT_END_EQ_ADDR((void*)ethh->h_source - sizeof(struct redir_opt), ethh->h_dest, ETH_ALEN);

  BPF_ASSERT_END_EQ((void*)&iph->tot_len - sizeof(struct redir_opt), __be16, bpf_htons(bpf_ntohs(iph->tot_len) + sizeof(struct redir_opt)));
  
  #ifdef KLEE_VERIFICATION
  struct redir_opt expected_ropt;
  expected_ropt.type = REDIR_OPT_TYPE;
  expected_ropt.size = 6;
  expected_ropt.ip = iph->daddr;
  expected_ropt.nop = 0x0101;
  #endif
  BPF_ASSERT_END_EQ((void *)(tcph + 1) - sizeof(struct redir_opt), struct redir_opt, expected_ropt);
  ...
  BPF_ASSERT_CONSTANT(&(iph->tot_len), sizeof(__be16));

  /* Change destination address */
  if (get_target(&iph->daddr))
    return XDP_ABORTED;
  BPF_ASSERT_CONSTANT(&iph->daddr, sizeof(__be32));
  
  if (update_macs(&iph->daddr, ethh))
    return XDP_ABORTED;
  BPF_ASSERT_CONSTANT(ethh, sizeof(struct ethhdr));

  /* Fix IP checksum */
  iph->check = 0;

  iph->check = ~csum_reduce_helper(bpf_csum_diff(0, 0, (__be32 *)iph, size, 0));
  BPF_ASSERT_CONSTANT(iph, sizeof(struct iphdr));
  ...
  BPF_ASSERT_CONSTANT(ptr, sizeof(struct redir_opt));
...
#pragma unroll
  ...
  BPF_ASSERT_CONSTANT(tcph, sizeof(struct tcphdr));
...
int xdp_prog_simple(struct xdp_md *ctx) {
  ...
  BPF_ASSERT_LEADS_TO_ACTION(nh_type != bpf_htons(ETH_P_IP), XDP_PASS);
  ...
  BPF_ASSERT_LEADS_TO_ACTION(ip_type != IPPROTO_TCP, XDP_PASS);
