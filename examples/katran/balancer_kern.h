  ...
  if (is_ipv6) {
    ...
    *pkt_bytes = bpf_ntohs(iph->tot_len);
    ...
    if (iph->frag_off & PCKT_FRAGMENTED) {
      VIGOR_TAG(TRAFFIC_CLASS, FRAGMENTED);
      // we drop fragmented packets.
      return XDP_DROP;
    }
    if (*protocol == IPPROTO_ICMP) {
      VIGOR_TAG(TRAFFIC_CLASS, ICMP);
...
process_packet(void *data, __u64 off, void *data_end, bool is_ipv6,
  ...
  __u16 pkt_bytes;
  action = process_l3_headers(&pckt, &protocol, off, &pkt_bytes, data, data_end,
                              is_ipv6);
  if (action >= 0) {
    return action;
  }
  ...
  if (protocol == IPPROTO_IPIP || protocol == IPPROTO_IPV6) {
    ...
    return process_encaped_ipip_pckt(&data, &data_end, xdp, &is_ipv6, &protocol,
                                     pass);
  }
  ...
  /* Lookup key contains destination IP, destination port, protocol */
  ...
  if (!vip_info) {
    ...
    VIGOR_TAG(TRAFFIC_CLASS, VIP_2ND_ATTEMPT);
    if (!(vip_info->flags & F_HASH_DPORT_ONLY)) {
  ...
  if (data_end - data > MAX_PCKT_SIZE) {
    VIGOR_TAG(TRAFFIC_CLASS,BIG_PACKET);
  ...
  if ((protocol == IPPROTO_UDP) && (vip_info->flags & F_QUIC_VIP)) {
    VIGOR_TAG(TRAFFIC_CLASS, QUIC);
    ...
    if (real_index > 0) {
      VIGOR_TAG(TRAFFIC_CLASS, GOOD_QUIC);
  ...
  if (!dst) {
    ...
    if (!(pckt.flags & F_SYN_SET) && !(vip_info->flags & F_LRU_BYPASS)) {
      ...
      VIGOR_TAG(TRAFFIC_CLASS, EXISTING_FLOW);
    }
    if (!dst) {
      VIGOR_TAG(TRAFFIC_CLASS, NEW_FLOW);
...
__attribute__((noinline)) int balancer_ingress(struct xdp_md *ctx) {
  ...
  if (eth_proto == BE_ETH_P_IP) {
    VIGOR_TAG(TRAFFIC_CLASS, IPV4);
    return process_packet(data, nh_off, data_end, false, ctx);
  } else if (eth_proto == BE_ETH_P_IPV6) {
    VIGOR_TAG(TRAFFIC_CLASS, IPV6);
