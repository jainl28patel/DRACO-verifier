...
static inline void biflow(struct flow_ctx_table_key *flow_key){
	...
	BPF_ASSERT("IP addresses ordered", flow_key->ip_src <= flow_key->ip_dst);
	BPF_ASSERT("L4 src & dst ordered", flow_key->l4_src <= flow_key->l4_dst);
}
...
int xdp_fw_prog(struct xdp_md *ctx)
{
	...
	if(ethernet->h_proto != BE_ETH_P_IP)
		goto EOP;
	BPF_ASSERT("Ethernet protocol is IP", ethernet->h_proto == BE_ETH_P_IP);
