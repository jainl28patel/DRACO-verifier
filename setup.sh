...
BUILDDIR="$(realpath -e "$CURRENTDIR"/dependencies)"
PATHSFILE="$CURRENTDIR/paths.sh"
...
{
	line "$PATHSFILE" 'KLEE_INCLUDE' "$CURRENTDIR/klee/include"
	line_multi "$PATHSFILE" 'PATH' "$CURRENTDIR/klee/build/bin:\$PATH"
	. "$PATHSFILE"
